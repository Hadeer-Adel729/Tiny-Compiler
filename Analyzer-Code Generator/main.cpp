...
struct SymbolTable
{
    ...
    void InsertVariable(const char* name, int line_num)
    {
...
void GeneratSymbolTable(TreeNode* node, SymbolTable* symbolTable){
    ...
    if (node->node_kind == READ_NODE || node->node_kind == ASSIGN_NODE || node->node_kind == ID_NODE ){
        symbolTable->InsertVariable(node->id, node->line_num);
    }
    ...
    if (node->child[0] != nullptr) {
        for (auto &child: node->child) {
            if (child != nullptr)
                GeneratSymbolTable(child, symbolTable);
        }
    }

    //Type Checking
    if(node->node_kind==IF_NODE && node->child[0]->expr_data_type!=BOOLEAN) {
        cout<<"Error in line "<<node->line_num<<": IF EXP Should be followed by a Boolean"<<endl;
        exit(0);
    }
    if(node->node_kind==WRITE_NODE && node->child[0]->expr_data_type!=INTEGER) {
        cout<<"Error in line "<<node->line_num<<": Write variable type should be Integer"<<endl;
        exit(0);
    }
    if(node->node_kind==ASSIGN_NODE && node->child[0]->expr_data_type!=INTEGER) {
        cout<<"Error in line "<<node->line_num<<": Assigned variable type should be Integer"<<endl;
        exit(0);
    }

    if(node->node_kind==REPEAT_NODE && node->child[1]->expr_data_type!=BOOLEAN) {
        cout<<"Error in line "<<node->line_num<<": Repeat EXP Should Be Boolean"<<endl;
        exit(0);
    }


    if (node->sibling != nullptr){// Analyze sibling node if found
        GeneratSymbolTable(node->sibling, symbolTable);
    }

}
/************************************* Code Generator ***********************************/
int GenerateExpressionCode(TreeNode* node, SymbolTable* symbolTable, int* Program_Variables){
    // base case
    if(node->node_kind == ID_NODE){
        // return the value of variable in RHS
        return Program_Variables[symbolTable->Find(node->id)->memloc];
    }
    if(node->node_kind == NUM_NODE){
        // return node number
        return node->num;
    }
    // if node not a ( NUM_NODE or ID_NODE ) --> Check Childs
    int x = GenerateExpressionCode(node->child[0], symbolTable, Program_Variables);
    int y = GenerateExpressionCode(node->child[1], symbolTable, Program_Variables);
...
void GenerateCode(TreeNode* node, SymbolTable* symbolTable, int* Program_Variables){

    if(node->node_kind == READ_NODE){
        cout<< "Enter "<<node->id<< ": ";
        int num ;
        cin >> num ;
        Program_Variables[symbolTable->Find(node->id)->memloc] = num ;
    }

    if(node->node_kind == WRITE_NODE){
        int num = GenerateExpressionCode(node->child[0], symbolTable, Program_Variables);
        //cout<<"Var: " << node->child[0]->id << " Val: "<<num<<endl;
        cout<< "Val: "<<num<<endl;
    }

    if(node->node_kind==REPEAT_NODE){
        do{ // repeat stmtseq until expr
            GenerateCode(node->child[0], symbolTable, Program_Variables);
        }
        while(!GenerateExpressionCode(node->child[1], symbolTable, Program_Variables));
    }

    if(node->node_kind == IF_NODE ){ // if expr then stmtseq [ else stmtseq ] end
        bool condition = GenerateExpressionCode(node->child[0], symbolTable, Program_Variables);
        if(condition){
            GenerateCode(node->child[1], symbolTable, Program_Variables);
        }
        else if(node->child[2]){
            GenerateCode(node->child[2], symbolTable, Program_Variables);
        }
    }
    ...
    if(node->sibling){ // check  sibling node if found
        GenerateCode(node->sibling, symbolTable, Program_Variables);
    }

}
void Run_Program(TreeNode* syntax_tree, SymbolTable* symbolTable){
    /* allocates an array of integers with a size equal to the number
    of variables in the program*/
    int* Program_Variables=new int[symbolTable->num_vars];

    /* initializes all elements of the array to 0. to ensure that variables
     start with a default value before the program execution.*/
    for(int i=0;i<symbolTable->num_vars;i++){
        Program_Variables[i] = 0;
    }
    GenerateCode(syntax_tree, symbolTable, Program_Variables);
    delete[] Program_Variables;
}
...
int main(){
    ...
    parseTree = Parse(&Tiny);
    SymbolTable symbolTable;
    GeneratSymbolTable(parseTree, &symbolTable);
    cout<<("\n---------- Symbol table ----------")<<endl;
    cout<<"\n";
    symbolTable.Print();
    cout<<"\n\n";
    ...
    cout<<("\n--------------------------------------");
    cout<<"\n---------- Run Program ---------------"<<endl;
    Run_Program(parseTree, &symbolTable);
    cout<<"------------------------------------------"<<endl;
    symbolTable.Destroy();
